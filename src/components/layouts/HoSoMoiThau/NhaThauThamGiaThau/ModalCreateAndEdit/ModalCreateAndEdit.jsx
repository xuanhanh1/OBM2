...
function ModalCreateAndEdit(props) {
  ...
  const [lstBidderResult, setLstBidderResult] = useState([]);
  ...
  useEffect(() => {
    if (_.isEmpty(id)) {
      ...
      setValue("bidding_document_id", id);
  ...
  useEffect(() => {
    if (!_.isEmpty(lstBidder)) {
      let result = lstBidder;
      let arrBidderId = lstBid.map((item) => item.bidder_id);

      for (let i = 0; i < arrBidderId.length; i++) {
        let newArr = _.remove(result, function (j) {
          return j.id === arrBidderId[i];
        });
      }

      setLstBidderResult(result);
    }
  }, [lstBidder]);
